 ...
 */
void MD5Pad(unsigned char *buf, unsigned long long len_buf, const unsigned char *input, unsigned long long len_input);

/*
 * The processing of a message chunk consists of four similar stages, termed rounds;
...
void MD5(unsigned char *output, const unsigned char *input, unsigned long long len_input) {
...
typedef unsigned int(*RoundFunctionType)(unsigned int X, unsigned int Y, unsigned int Z); //定义函数类型
static RoundFunctionType RoundFunction[4] = {F, G, H, I}; //定义并初始化计算函数指针数组
