...
void geneKPlus(unsigned char *KPlus,const unsigned char *key, unsigned int keylen) {
...
unsigned char * connectUnsignedCharString(unsigned char *dest, const unsigned char *str1, unsigned long long len1, const unsigned char *str2, unsigned long long len2) {
    memcpy(dest, str1, len1);
    memcpy(&dest[len1], str2, len2);
    return dest;
}

void HMAC_MD5(unsigned char *output, const unsigned char *key, unsigned int keylen, const unsigned char *str, unsigned long long len) {
    unsigned char *Kplus = (unsigned char *) malloc ((BLOCK_SIZE / 8) * sizeof(unsigned char));
    geneKPlus(Kplus, key, keylen);

    unsigned char *Si = (unsigned char *) malloc ((BLOCK_SIZE / 8 + len) * sizeof(unsigned char));
    geneSi(Si, Kplus);

    unsigned char *Si_M = Si;
    connectUnsignedCharString(Si_M, Si, (BLOCK_SIZE / 8), str, len);

    unsigned char *H_Si_M = (unsigned char *) malloc ((OUTPUT_SIZE / 8) * sizeof(unsigned char));
    MD5(H_Si_M, Si_M, (BLOCK_SIZE / 8 + len));

    unsigned char *So = (unsigned char *) malloc ((BLOCK_SIZE / 8 + (OUTPUT_SIZE / 8)) * sizeof(unsigned char));
    geneSo(So, Kplus);

    unsigned char *So_H_Si_M = So;
    connectUnsignedCharString(So_H_Si_M, So, (BLOCK_SIZE / 8), H_Si_M, (OUTPUT_SIZE / 8));
